...
/**
 * <p>
 * Wait for the given matcher to evaluate to true. It is required that the embedded matcher checks
 * a mutable aspect of the matched object.
 * </p>
 * <p>
 * This matcher uses a decelerating wait approach, i. e. it tries to poll the state very often at
 * the beginning of the wait period and slows down towards the end. As a result fast system under
 * tests will finish very quickly while slower systems are given more time to actually reach the
 * wanted state. If the state is reached directly the algorithm will not even wait at all.
 * </p>
 * <dl>
 * <dt><strong>Convenience Function Included:</strong></dt>
 * <dd>
 * <p>
 * Most of the time using this matcher you will have a component under test which changes its state
 * over time. For the assertion you will need to retrieve the state of this component repeatedly
 * until it meets your required conditions.
 * </p>
 * <p>
 * Therefore this matcher comes with two inner classes ready to be filled with your state
 * retrieving function and to compare the retrieved state with any normal matcher available in the
 * library. If you rely on Guava or Java 8 you might as well use the functional interfaces they
 * provide. So this is just for convenience if you happen not to have Guava and/or Java 8 at hand.
 * </p>
 * </dd>
 ...
 * <dd>
 * Unlike other matchers it is not useful to combine this matcher with e. g. logical operations
 * like for example <em>not</em>. Such logical operations need to be placed in the embedded matcher
 * instead.
 * </dd>
 * </dl>
 *
...
public class WaitFor<T> extends TypeSafeMatcher<T> {
   ...
   * <dd>
   * <p>
   * As the item tested here is expected to change over time it might result in a mismatch
   * description
   * which does not match the invalid state retrieved before. Thus you might want to store the
   * state
  ...
  private long sleepAndRecalculateDelay(long previousDelay,
    ...
    try {
      sleep(Math.max(1, Math.min(newDelay, deadlineTimeMs + SLEEP_NOT_MUCH_LONGER_OFFSET_MILLIS
                                           - afterEvaluationTimeMs)));
